...
svc_status(){
  # prints Active/Inactive tag for a systemd unit
...
protocols(){
  while true; do
    ...
    echo -e "${O}--- TUNNELLING PROTOCOLS ---${Z}"
    printf " [ 1] 🚀 Install badvpn (UDP 7300)          %s\n" "$(svc_status nexuspacket-badvpn)"
    echo   " [ 2] 🗑  Uninstall badvpn"
    printf " [ 3] 🚀 Install udp-custom                 %s\n" "$(svc_status nexuspacket-udpcustom)"
    echo   " [ 4] 🗑  Uninstall udp-custom"
    printf " [ 5] 🔒 Install HAProxy + Nginx edge stack  %s\n" "$(svc_status haproxy)"
    echo   " [ 6] 🗑  Uninstall HAProxy + Nginx edge stack"
    printf " [ 7] 🎯 Install/View DNSTT (Port 53)        %s\n" "$(svc_status nexuspacket-dnstt)"
    echo   " [ 8] 🗑  Uninstall DNSTT"
    printf " [ 9] 🌐 Install NexusPacket Proxy (WS relay)%s\n" "$(svc_status nexuspacket-proxy)"
    echo   " [10] 🗑  Uninstall NexusPacket Proxy"
    echo   " [11] 📊 Service status (all)"
    echo -e "${O}--- MANAGEMENT PANELS ---${Z}"
    echo   " [12] 💻 Install X-UI / 3X-UI Panel"
    echo   " [13] 🗑  Uninstall X-UI / 3X-UI Panel"
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo   " [ 0] ↩  Return"
    read -r -p "Choice: " c
    case "$c" in
      1) install_badvpn;;
      2) uninstall_badvpn;;
      3) echo -e "${Y}udp-custom does not have a verified open-source upstream yet.${Z}"; echo "Tell me the exact project you want and I'll wire it in properly, or I can write an original UDP gateway from scratch."; read -r -p "Enter..." _;;
      4) systemctl disable --now nexuspacket-udpcustom 2>/dev/null || true; echo "Removed (if present)."; read -r -p "Enter..." _;;
      5) install_pkg haproxy nginx; systemctl enable --now nginx haproxy; echo "HAProxy/Nginx installed. Configure routing before exposing production traffic."; read -r -p "Enter..." _;;
      6) systemctl disable --now haproxy nginx 2>/dev/null || true; apt-get purge -y haproxy nginx nginx-common >/dev/null 2>&1 || true; echo "Removed."; read -r -p "Enter..." _;;
      7) install_dnstt;;
      8) uninstall_dnstt;;
      9) install_nexuspacket_proxy;;
      10) uninstall_nexuspacket_proxy;;
      11) systemctl --no-pager --type=service --state=running | grep -E 'haproxy|nginx|ssh|nexuspacket' || true; read -r -p "Enter..." _;;
      12) echo "Official 3X-UI installer is upstream-managed. Run only after reviewing its release/source."; echo "https://github.com/MHSanaei/3x-ui"; read -r -p "Install now? [y/N] " x; [[ "$x" =~ ^[Yy]$ ]] && bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/3x-ui/master/install.sh);;
      13) echo "Run the official 3X-UI uninstall from its own menu (x-ui uninstall)."; read -r -p "Enter..." _;;
      0) return;;
    esac
  done
}
...
install_badvpn(){
  show_banner
  echo -e "${P}--- Installing badvpn-udpgw (UDP Gateway) ---${Z}"
  read -r -p "UDP gateway port [7300]: " port; port=${port:-7300}
  install_pkg build-essential cmake git >/dev/null 2>&1
  if [[ ! -x /usr/local/bin/badvpn-udpgw ]]; then
    echo "Building badvpn from source (ambrop72/badvpn, official upstream)..."
    rm -rf /tmp/badvpn-build
    git clone --depth 1 https://github.com/ambrop72/badvpn.git /tmp/badvpn-build || {
      echo -e "${R}Clone failed. Check network access to github.com.${Z}"; read -r -p "Enter..." _; return
    }
    mkdir -p /tmp/badvpn-build/build && cd /tmp/badvpn-build/build || return
    cmake .. -DBUILD_NOTHING_BY_DEFAULT=1 -DBUILD_UDPGW=1 >/dev/null
    make -j"$(nproc)" >/dev/null
    install -m 0755 udpgw/badvpn-udpgw /usr/local/bin/badvpn-udpgw
    cd - >/dev/null || true
  fi
  cat > /etc/systemd/system/nexuspacket-badvpn.service <<EOF
[Unit]
Description=NexusPacket badvpn-udpgw (UDP gateway)
...
EOF
  ...
  systemctl enable --now nexuspacket-badvpn.service
  echo -e "${G}badvpn-udpgw running on 127.0.0.1:$port${Z}"
  read -r -p "Enter..." _
}
uninstall_badvpn(){
  systemctl disable --now nexuspacket-badvpn.service 2>/dev/null || true
  rm -f /etc/systemd/system/nexuspacket-badvpn.service
  systemctl daemon-reload
  echo "badvpn-udpgw removed."
  read -r -p "Enter..." _
}

install_dnstt(){
  show_banner
  echo -e "${P}--- Installing DNSTT (DNS tunnel server) ---${Z}"
  echo "Upstream: https://www.bamsoftware.com/software/dnstt/ (official, open source, MIT-derived)"
  read -r -p "Proceed with build from official source? [y/N] " x
  [[ "$x" =~ ^[Yy]$ ]] || return
  install_pkg golang-go git >/dev/null 2>&1
  rm -rf /tmp/dnstt-build
  git clone https://www.bamsoftware.com/git/dnstt.git /tmp/dnstt-build 2>/dev/null || \
    git clone https://github.com/dnstt/dnstt.git /tmp/dnstt-build 2>/dev/null || {
      echo -e "${R}Could not fetch dnstt source. Check network access.${Z}"; read -r -p "Enter..." _; return
  }
  ( cd /tmp/dnstt-build/dnstt-server 2>/dev/null || cd /tmp/dnstt-build; go build -o /usr/local/bin/dnstt-server . ) || {
    echo -e "${R}Build failed.${Z}"; read -r -p "Enter..." _; return
  }
  echo -e "${G}dnstt-server built at /usr/local/bin/dnstt-server${Z}"
  echo "You still need to generate keys and configure your NS delegation manually — this is domain-specific."
  read -r -p "Enter..." _
}
uninstall_dnstt(){
  systemctl disable --now nexuspacket-dnstt 2>/dev/null || true
  rm -f /usr/local/bin/dnstt-server /etc/systemd/system/nexuspacket-dnstt.service
  systemctl daemon-reload
  echo "dnstt removed."
  read -r -p "Enter..." _
}
